 ...
 */
package org.opendaylight.yang.gen.v1.urn.ietf.params.xml.ns.yang.ietf.inet.types.rev251222;
...
public final class IetfInetUtil {
    ...
    private static final Pattern HOST_IPV6_PATTERN2 = Pattern.compile(
        "(([^:]+:){6}(([^:]+:[^:]+)|(.*\\..*)))|((([^:]+:)*[^:]+)?::(([^:]+:)*[^:]+)?)(%.+)?");
    private static final Pattern HOST_NAME_PATTERN = Pattern.compile("[a-zA-Z0-9\\-\\.]+");
    ...
    public static Host hostFor(final String str) {
        ...
        final var nameMatcher = HOST_NAME_PATTERN.matcher(str);
        ...
        if (ipv4Matcher.matches()) {
            ...
            return new Host(new HostName(str));
        } else {
